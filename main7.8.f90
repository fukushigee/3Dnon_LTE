...
! Real*8 C(NSPECIESMAX,-1:29 +1,-1:29 +1) &
      ...
      real*8 :: alpha(10,ixmax+1,iymax+1,izmax+1)
      real*8 :: S(10,ixmax+1,iymax+1,izmax+1)
      real*8 :: n(0:10,ixmax+1,iymax+1,izmax+1)
...
! lev+60=60,61,... disp
! lev+10=10,11,... Tr
! 500, 10 are changed to variables in cpp
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      Loop=1
      open(readn32,file='level3-2.d')
      open(readn98,file='level9-8.d')

      do Loop=1,Loopmax

      write(readn32,*),loop,n(2,64,44,64)

      write(readn98,*),loop,n(8,64,44,64)
...
!-------------------------------------------------------------------!
      ...
      use state_inc
      implicit real*8(a-h,o-z)
      parameter(levmax=10,ijkm=ixm*iym*izm)
      common /density/ Xc
...
! close(1)
      ...
      tk = tem !kintec temperature



      print*, 'ng=', ng, 'izmax=', izmax
      print*,'max, min rhoo()', maxval(rho), minval(rho)
      ...
      !print*,'n(H2)(64,44,64)',rho(65,45,65)
...
!======================================================================!
      ...
      use state_inc
      implicit real*8(a-h,o-z)
      parameter(Nray0=500,levmax=10,ixm2=ixm+2,iym2=iym+2,izm2=izm+2, &
...
! Slave Solve_I_0
      ...
      parameter (Nray0=500,Lmax=ng*3,levmax=10, ixm2=ixm+2,iym2=iym+2,izm2=izm+2)
      common /grid/ xf(0:ixm2), yf(0:iym2), zf(0:izm2), dx0, eps, xcent, ycent, zcent
      common /const/ h,lc,bc
      real*8 lc
! velocity field
...
! print*,'LEVMAX in get_c', 10, levmax
...
!? enddo
      do i=0, levmax
         do j =0, levmax
            if(i.ne.j) then
               if((CTABLE(ilabel-1,i,j).le.tiny).OR. & !tiny=1.d0-180
